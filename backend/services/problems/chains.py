...
def _attempt(
    ...
    - ``library``：**题库同尺**——正解 ≥ ``chain_library_min_winrate``（默认 0.6，
      古典题入库线）且正解 − 次优 ≥ ``chain_library_min_gap``（默认 0.15），
      死活/对杀另需 正解 − 脱先 ≥ ``chain_relative_urgency``；用于从古典死活
      题集挑出来的种子（19 路局部题在整盘尺度上过不了 0.95，但按题库同一把
      尺子是合格题）；
    ...
    if mode in ("local_death", "local_board"):
        ...
        # 题库同尺（v1.7.4）：古典题入库用的就是这把尺子——正解 ≥
        # ``chain_library_min_winrate``（默认 0.6＝MIN_ANSWER_WINRATE），
        # 且正解 − 次优 ≥ ``chain_library_min_gap``（默认 0.15＝答案明显更优）；
        # 死活/对杀另需 正解 − 脱先 ≥ ``chain_relative_urgency``（默认 0.3）。
        # 用途：从古典死活题集挑出来的种子（19 路局部题在小棋盘上也过不了
        # 0.95，但按题库同一把尺子是合格题）。
        ...
        if best_wr < float(lib_min):
            return None
        if best_wr - second_wr < float(lib_gap):
            return None                       # 答案不够突出
        if urgent and pass_wr is not None and best_wr - pass_wr < float(rel_urgency):
            return None                       # 可以脱先
    elif mode == "relative":
...
def _problem_row(
    ...
    branches = {
        ...
        # relative / library 口径：正解−次优 / 正解−脱先 的胜率差
...
def grow_chain(
    ...
    if mode not in ("local_board", "local_death", "relative", "library", "winrate"):
        mode = "local_board"
    ...
    lib_min = float(cfg.get("chain_library_min_winrate", 0.6))
    lib_gap = float(cfg.get("chain_library_min_gap", 0.15))
    ...
    while node is not None and node.depth < int(max_depth):
        ...
        for variant in _variants(node, size, max_per_level):
            outcome = _attempt(
                ...
                rel_gap, rel_urgency, lib_min, lib_gap,
            )
            if outcome is None:
                discarded += 1
                continue
